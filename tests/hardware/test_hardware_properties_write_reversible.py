...
def test_hardware_property_write_reversible_generic_api(hardware_write_guard, hardware_config) -> None:
    ...
    write_tolerance = max(abs(report.requested_value) * 0.1, 0.5)
    restore_tolerance = max(abs(report.before.abs_value) * 0.1, 0.5)
    assert abs(report.written.abs_value - report.requested_value) <= write_tolerance
